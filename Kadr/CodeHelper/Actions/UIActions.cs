namespace APG.CodeHelper.Actions
{
    ...
    {
        taAdd, taDelete, taCopy, taCut, taUpdate, taPaste, taExport, taAddHistory, taHistory
    }
    ...
    public abstract class UIAction
    {
        ...
        /// <summary>
        /// Переопределите в производных классах для выполнения операции экспорта данных объекта
        /// </summary>
        /// <param name="sender">Определяет объект, пославший сообщение</param>
        protected abstract void ExportExecute(object sender);
        /// <summary>
        /// Переопределите в производных классах для выполнения операции добавления истории объекта
        /// </summary>
        /// <param name="sender">Определяет объект, пославший сообщение</param>
        protected abstract void AddHistoryExecute(object sender);
        /// <summary>
        /// Переопределите в производных классах для выполнения операции просмотра истории объекта
        /// </summary>
        /// <param name="sender">Определяет объект, пославший сообщение</param>
        protected abstract void HistoryExecute(object sender);
        ...
        /// <summary>
        /// Переопределите в производных классах для определения возможности выполнить
        /// добавление истории (изменения) объекта
        /// </summary>
        /// <param name="sender">Определяет объект, пославший сообщение</param>
        /// <returns>true, если опрация допустима, false - в противном случае</returns>
        protected abstract bool CanAddHistory(object sender);
        /// <summary>
        /// Переопределите в производных классах для определения возможности 
        /// просмотреть историю объекта
        /// </summary>
        /// <param name="sender">Определяет объект, пославший сообщение</param>
        /// <returns>true, если опрация допустима, false - в противном случае</returns>
        protected abstract bool CanHistory(object sender);
        /// <summary>
        /// Переопределите в производных классах для определения возможности выполнить
        /// экспорт объекта
        /// </summary>
        /// <param name="sender">Определяет объект, пославший сообщение</param>
        /// <returns>true, если опрация допустима, false - в противном случае</returns>
        protected abstract bool CanExport(object sender);
        ...
        public void Execute(object sender, UIObjectAction index)
        {
            switch (index)
            {
                ...
                case UIObjectAction.taExport: ExportExecute(sender); break;
                case UIObjectAction.taAddHistory: AddHistoryExecute(sender); break;
                case UIObjectAction.taHistory: HistoryExecute(sender); break;
            }
        }
        ...
        public bool CanDoThis(object sender, UIObjectAction index)
        {
            switch (index)
            {
                ...
                case UIObjectAction.taExport: return CanExport(sender);
                case UIObjectAction.taAddHistory: return CanAddHistory(sender);
                case UIObjectAction.taHistory: return CanHistory(sender);
