...
namespace Kadr.Controllers
{
    ...
    class ExcelExportController
    {
        ...
        public void ExportDepartmentsToExcel(int idDepartment)
        {
            ...
            (ws.Cells[RowNumber, 2] as Range).Value2 = "Руководящий отдел";
            (ws.Cells[RowNumber, 3] as Range).Value2 = "Отдел";
            foreach (GetFullSubDepartmentsResult subDep in subDeps)
            {
                RowNumber++;
                (ws.Cells[RowNumber, 1] as Range).Value2 = subDep.ManagerName;
                (ws.Cells[RowNumber, 2] as Range).Value2 = subDep.DepartmentManagerName;
                (ws.Cells[RowNumber, 3] as Range).Value2 = subDep.DepartmentName;
            }


            //итоговая настройка
            ws.Range["A1", "C" + RowNumber.ToString()].Columns.AutoFit();
            ws.Range["A1", "C" + RowNumber.ToString()].Borders.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Black);
            Excel.Visible = true;
            Excel.ReferenceStyle = RefStyle;
            ReleaseExcel(Excel as Object);
        }
